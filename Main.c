...
int main() {
  ...
  while (1) {
    ...
    for (int i=0; i<32; i++) {
      ...
      if (i%8 == 0) {
        Packet generalSensorPacket = generateGeneralSensorPacket(iteration, packetsCounter, i, accelerationReadings, 
            gyroscopeReadings, magnetometerReadings, uv1Readings);
      ...
      //Read from all 1 byte sensors here if there are any. array[i] = ...
    ...
    /*-----------------------------------------------------------------------------*/
    /*------------------------------------ UV1 ------------------------------------*/
    sensorPacket = generateUV(uv1Readings, iteration, packetsCounter);
    packetsCounter = enqueuePacket(packetsCounter, serialBusQueue, sensorPacket);
